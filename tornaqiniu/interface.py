...
def encode_entry(method):
	...
	def wrapper(self,*args,**kwargs):
		if len(args)==2:
			encoded_entry=bytes_decode(urlsafe_base64_encode(args[1]+':'+args[0]))
			return method(self,encoded_entry)
		elif len(args)==4:	
			src_entry=bytes_decode(urlsafe_base64_encode(args[1]+":"+args[0]))
			dest_entry=bytes_decode(urlsafe_base64_encode(str(args[3])+":"+str(args[2])))
...
class QiniuInterface(object):
	...
	def image_watermark(cls,water_img_url,**kwargs):
		...
		interface+='/image/'+str(bytes_decode(urlsafe_base64_encode(water_img_url)))
		if len(kwargs)>0:
		...
		if len(kwargs)>0: 
			...
			for field,value in kwargs.items():
				...
				if field in urlsafe_encode_fields:
					interface+='/'+str(field)+'/'+str(bytes_decode(urlsafe_base64_encode(value)))
				else:
					interface+='/'+str(field)+'/'+str(value)
		return ("",interface)
	@classmethod
	def image_view2(cls,mode,width=None,height=None,**kwargs):
		...
		if len(kwargs)>0:
			valid_fields=set(['format','interlace','quality','ignor-error'])
			for field,value in kwargs.items():
				if field not in valid_fields:
					raise Exception("Not support field '%s'"%field)
				interface+='/'+str(field)+'/'+str(value)
		...
		if len(kwargs)>0:
			...
			for field,value in kwargs.items():
				if field not in urlsafe_encode_fields:
					...
					interface+'/'+str(field)+'/'+bytes_decode(urlsafe_base64_encode(value))
		return ("",interface)
	@classmethod
